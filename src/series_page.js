...
class Series_page extends Component {
	...
	getNextGame = () => {
		axios
			.get(
				`https://cors-anywhere.herokuapp.com/https://data.nba.net/data/10s/prod/v1/2017/teams/${this.props.location.state.series.topRow.teamId}/schedule.json`
			)
			.then(resp => {
				if (resp.status === 200) {
	...
	componentWillMount() {
		...
		this.setState(
			{
				...
				topTeam: series_info.topTeam,
				bottomTeam: series_info.bottomTeam
			},
			() => {
				if (!this.state.series.isSeriesCompleted) {
                    this.getNextGame();
				}
			}
		);
		...
		YTSearch(
			{
				key: "AIzaSyDNGakh-vKUDZBEKzVbpmYPu3yWYt1fnTw",
				term: `${series_info.topTeam} ${series_info.bottomTeam} game ${game_num}`
			},
			videos => {
		...
		return (
			<div className="series_page_cover">
				<div className="series-info">
					<button
						className="back-button btn btn-danger"
				...
				<div className="team-info">
					({topRow.seedNum}){topTeam}
					<div>wins: {topRow.wins}</div>
				</div>
				<div className="team-info">
					({bottomRow.seedNum}){bottomTeam}
				...
				<div className="contain-youtube">
					...
					{this.state.videos.map(video => {
						...
						return (
							<div
								key={video.etag}
								className="contain-vid embed-responsive embed-responsive-16by9"
							>
								<iframe
									title={video.etag}
